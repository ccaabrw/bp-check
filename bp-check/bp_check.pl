...
# 29Jun07 The 'dummy' MAC address is no longer considered in the automated list of duplicates
#	  (to prevent confusion when people apply patches to bp_check.known)
# 13Mar13 Allow helper addresses to be denoted using 'helper.addr'
#
# TODO:
...
sub print_nice {
    ...
    if ($nice_all) {
        ...
        foreach $host (sort keys %info) {
            if ($info{$host}{ipaddr} eq $no_name && $host ne $help_name) {
        ...
        foreach $host (sort keys %host_name) {
            if (@{$host_name{$host}} > 1 && $host ne $help_name) {
	        printf("%-24s", $host);
	        $tab = "";
...
sub print_data {
    ...
    if (!$data_server) {
        ...
        foreach $addr (sort keys %hw_addr) {
            if ($addr ne "XXXXXXXXXXXX" && @{$hw_addr{$addr}} > 1) {
	        foreach $mach (sort @{$hw_addr{$addr}}) {
                    printf("MAC   %-14s%s\n", $addr, $mach);
	        }
            }
